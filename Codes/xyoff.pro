...
;      /QUIET = turns off printing out of calculated offset values
; OUTPUTS:
...
function xyoff,a,b,bx,by,wdow,mask,hx,hy,fna,$
               sobel=sobel,quiet=quiet,help=help
  ...
  if n_elements(fna) eq 0 then begin
    ...
    if keyword_set(sobel) eq 0 then na=(na-mean(na))*wdow $
  ...
  if keyword_set(sobel) eq 0 then nb=(nb-mean(nb))*wdow $
